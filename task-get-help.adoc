...
.Avant de commencer
* Pour utiliser la fonction *Créer un cas*, vous devez d'abord associer vos informations d'identification du site de support NetApp à votre identifiant BlueXP . https://docs.netapp.com/us-en/bluexp-setup-admin/task-manage-user-credentials.html["Découvrez comment gérer les identifiants associés à votre connexion BlueXP"^]
...
** Modifiez les colonnes qui apparaissent dans le tableau en sélectionnant, puis en image:https://raw.githubusercontent.com/NetAppDocs/bluexp-family/main/media/icon-table-columns.png["Icône plus qui apparaît dans le tableau"] choisissant les colonnes que vous souhaitez afficher.
+
...
. Gérer un dossier existant en sélectionnant et en image:https://raw.githubusercontent.com/NetAppDocs/bluexp-family/main/media/icon-table-action.png["Icône avec trois points qui apparaît dans la dernière colonne du tableau"] sélectionnant l'une des options disponibles :
+
